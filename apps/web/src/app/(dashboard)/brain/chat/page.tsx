...
import { Brain, Send, Plus, Trash2, ChevronDown, User, Loader2, MessageSquare } from 'lucide-react'

interface Message { id: string; role: 'user' | 'assistant'; content: string; createdAt: string }
interface Conversation { id: string; title: string | null; messageCount: number; agent: { name: string; slug: string } }
interface Agent { id: string; slug: string; name: string; title: string; color: string | null }
...
function ChatContent() {
  ...
  async function sendMessage() {
    ...
    const optimistic: Message = { id: Date.now().toString(), role: 'user', content: userMsg, createdAt: new Date().toISOString() }
    ...
    try {
      const r = await fetch('/api/v1/brain/chat', {
        ...
        body: JSON.stringify({ message: userMsg, agentSlug: selectedAgent, conversationId: currentConvId }),
      })
      const d = await r.json()
      if (d.success) {
      ...
      <div className="w-64 border-r border-gray-200 bg-white flex flex-col">
        ...
        <div className="p-3 border-b border-gray-100">
          ...
          <div className="relative">
            <button onClick={() => setAgentDropdown(v => !v)}
            ...
            {agentDropdown && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-10 max-h-60 overflow-y-auto">
                {agents.map(a => (
                  <button key={a.id} onClick={() => { setSelectedAgent(a.slug); setAgentDropdown(false) }}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {loadingMessages ? (
            ...
            <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              {msg.role === 'assistant' && (
                <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shrink-0 mt-0.5">
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-white">
          <div className="flex items-end gap-3 bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 focus-within:border-indigo-400 focus-within:bg-white transition-all">
            ...
            <button onClick={sendMessage} disabled={!input.trim() || sending}
              className="w-8 h-8 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-200 text-white rounded-xl flex items-center justify-center transition-colors shrink-0">
